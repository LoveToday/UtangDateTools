...
- (NSString *)stringMonthDayTime
{
    ...
    if (chaDay == 0) {
        str = [NSString stringWithFormat:@"今天 %02zd:%02zd",self.hour,self.minute];
    }else if (chaDay == 1){
        str = [NSString stringWithFormat:@"明天 %02zd:%02zd",self.hour,self.minute];
    }else if (chaDay == -1){
        str = [NSString stringWithFormat:@"昨天 %02zd:%02zd",self.hour,self.minute];
    }else{
        NSDateFormatter *formatter = [[NSDateFormatter alloc]init];
...
+ (NSString *)stringCompareTimeWith:(NSString *)str
{
    if (str.length<16) {
        return nil;
    }
    NSDate *date = [NSDate dateFromString:[str substringToIndex:10] withFormat:[NSDate dateFormatString]];
    NSString *day = [date stringYearMonthDayCompareToday];
    NSString *time = [str substringWithRange:NSMakeRange(11, 5)];
    return [NSString stringWithFormat:@"%@ %@",day,time];
}

- (NSString *)stringTimeCompareToday
{
    NSString *str;
    NSInteger chaDay = [self daysBetweenCurrentDateAndDate];
    if (chaDay == 0) {
...
- (NSString *)stringCutSubSeconds {
    return [self stringWithFormat:[NSDate timeFormatSubString]];
}
